...
1. leaf dry weight and SLA 
2. shoot and root mass
3. total mass and SLA

total mass + corr w leaf dry weight (0.46)
shoot mass strongly + corr total mass (0.902)
shoot mass + corr w root mass (0.553)
leaf dry mass and SLA not corr (0.003)
...
plants with higher biomass seem to have large variances 
grindelia has highest, acmispon has lowest 
...
well watered plants have higher biomass than drought stressed
...
larger SLA doesn't really correlate with higher biomass

# Model construction

What is the null model?  

0. **no predictors: total mass isn't predicted by any of the predictors**  

What is the saturated model?  

1. **all 3 predictors: total mass is predicted by SLA, water treatment, and species**  
...
models_table <- flextable(models)
```

Out of the four models we have tried, the best model is the one that includes **all 3 predictors**  
...
- reference for `water_treatment`  = "drought water_treatment Drought stressed"
- reference for `species_name`  = "Acmispon glaber (deerweed) species_nameAcmispon glaber"
...
- intercept: **deerweed mass in drought stressed treatment when SLA = 0**  
- SLA slope: **across drought treatments and species, w each 1 unit in in SLA, expect in om total mass of -0.00025 which means with estimate and SE< there is not effect of SLA on total mass** 
estimate is super smaller, and SE overlaps 0, small t, large p
